...
{   
    ...
    function export_wp( $args = array() ) {
        ...
        $this->WXR_VERSION = defined('WXR_VERSION') ? WXR_VERSION : 1.2;
        ...
        $post_ids = $wpdb->get_col(
            $wpdb->prepare(  "SELECT ID FROM {$wpdb->posts} WHERE {$wpdb->posts}.ID IN (%s)", $id_list )
        );
    ...
    <rss version="2.0"
        xmlns:excerpt="http://wordpress.org/export/<?php esc_attr_e($this->WXR_VERSION); ?>/excerpt/"
        xmlns:content="http://purl.org/rss/1.0/modules/content/"
        xmlns:wfw="http://wellformedweb.org/CommentAPI/"
        xmlns:dc="http://purl.org/dc/elements/1.1/"
        xmlns:wp="http://wordpress.org/export/<?php esc_attr_e($this->WXR_VERSION) ?>/"
    >
    ...
    <channel>
        ...
        <wp:wxr_version><?php esc_attr_e($this->WXR_VERSION); ?></wp:wxr_version>
        ...
        <wp:category>
            <wp:term_id><?php esc_html_e((int)$c->term_id); ?></wp:term_id>
        ...
        <wp:tag>
            <wp:term_id><?php echo esc_html_e((int)$c->term_id); ?></wp:term_id>
        ...
        if ( $post_ids ) {
            ...
            while ( $next_posts = array_splice( $post_ids, 0, 20 ) ) {
                $the_post_list = implode( ',', $next_posts );
                $posts = $wpdb->get_results( 
                    $wpdb->prepare("SELECT * FROM {$wpdb->posts} WHERE ID IN (%s)", $the_post_list )
                );
                ...
                foreach ( $posts as $post ) {
                     ...
                     */
                    $title = $this->wxr_cdata( esc_html(apply_filters( 'the_title_export', $post->post_title )) );
                     ...
                     */
                    $content = $this->wxr_cdata( esc_html(apply_filters( 'the_content_export', $post->post_content )) );
                     ...
                     */
                    $excerpt = $this->wxr_cdata( esc_html(apply_filters( 'the_excerpt_export', $post->post_excerpt )));
        ...
        <item>
            ...
            <wp:post_id><?php esc_html_e((int) $post->ID); ?></wp:post_id>
            ...
            <wp:post_parent><?php esc_html_e( (int) $post->post_parent);?></wp:post_parent>
            <wp:menu_order><?php esc_html_e( (int) $post->menu_order); ?></wp:menu_order>
            ...
            <wp:is_sticky><?php esc_html_e( (int) $is_sticky );?></wp:is_sticky>
        ...
        endforeach;
                    ...
                    foreach ( $comments as $c ) :
                        ?>
            <wp:comment>
                <wp:comment_id><?php esc_html_e( (int) $c->comment_ID); ?></wp:comment_id>
                ...
                <wp:comment_author_url><?php esc_html_e( esc_url_raw( $c->comment_author_url )); ?></wp:comment_author_url>
                ...
                <wp:comment_parent><?php esc_html_e( (int) $c->comment_parent); ?></wp:comment_parent>
                <wp:comment_user_id><?php esc_html_e( (int) $c->user_id); ?></wp:comment_user_id>
                        <?php
        ...
        function wxr_authors_list( array $post_ids = null ) {
            ...
            $results = $wpdb->get_results( 
                $wpdb->prepare(
                    "SELECT DISTINCT post_author FROM $wpdb->posts WHERE post_status != 'auto-draft' %s", 
                    $and
                )
            );
            ...
            foreach ( $authors as $author ) {
                echo "\t<wp:author>";
                echo '<wp:author_id>' . esc_html((int) $author->ID) . '</wp:author_id>';
        ...
        function wxr_nav_menu_terms() {
            ...
            foreach ( $nav_menus as $menu ) {
                echo "\t<wp:term>";
                echo '<wp:term_id>' .esc_html((int) $menu->term_id) . '</wp:term_id>';
